...
def get_date_822():
    ...
    return time.strftime(
        calendar.day_abbr[t.tm_wday] + ", %d " +
        calendar.month_abbr[t.tm_mon] + " %Y %H:%M:%S %z", t)


def get_version_str(pkg):
    args = ['/usr/bin/dpkg-query', '--show', '--showformat=${Version}', pkg]
...
def get_deb_depends_from_setuptools_requires(requirements, on_failure="warn"):
    ...
    egginfore = (r"(/(%s)(?:-[^/]+)?(?:-py[0-9]\.[0-9.]+)?\.egg-info)"
                 % '|'.join(
                     req.project_name.replace('-', '_')
        ...
        if not gooddebs:
            if on_failure == 'warn':
                ...
                raise CantSatisfyRequirement(
                    "I found no Debian package which provides the required "
                    "Python package \"%s\" with version requirements "
                    "\"%s\"." % (req.project_name, req.specs), req)
...
def apt_cache_info(apt_cache_cmd, package_name):
    ...
    if returncode:
        ...
        if not (
            returncode == 100 and errline ==
            "E: You must put some 'source' URIs in your sources.list\n"
        ):
            log.error('ERROR running: %s', ' '.join(args))
            raise RuntimeError(
                'returncode %d from subprocess %s' % (returncode, args))
...
class DebianInfo:
    ...
    def __init__(self,
        ...
        if cfg_files is NotGiven:
            raise ValueError("cfg_files must be supplied")
        if module_name is NotGiven:
            raise ValueError("module_name must be supplied")
        if default_distribution is NotGiven:
            raise ValueError("default_distribution must be supplied")
        if guess_maintainer is NotGiven:
            raise ValueError("guess_maintainer must be supplied")
        if upstream_version is NotGiven:
            raise ValueError("upstream_version must be supplied")
        if has_ext_modules is NotGiven:
            raise ValueError("has_ext_modules must be supplied")
        if description is NotGiven:
            raise ValueError("description must be supplied")
        if long_description is NotGiven:
            raise ValueError("long_description must be supplied")
        if homepage is NotGiven:
            raise ValueError("homepage must be supplied")
        ...
        if forced_upstream_version == '':
            ...
            if (
                ...
                raise ValueError(
                    'forced upstream version ("%s") not a '
        ...
        if has_ext_modules:
            ...
            if with_python2:
                build_deps.append(
                    'python-all-dev (>= %s)' % PYTHON_ALL_MIN_VERS)
        ...
        if self.patch_level != '':
            if patch_level is not None:
                raise RuntimeError('A patch level was specified on the '
                                   'command line and in .cfg file.')
            else:
        ...
        python2_depends_name = parse_val(
            cfg, module_name, 'Python2-Depends-Name')
...
\tdh binary --after dh_shlibdeps""" % dpkg_shlibdeps_params
        ...
        if guess_conflicts_provides_replaces:
            ...
            for version_info in apt_cache_info('showsrc', self.package):
                ...
                # Remember each of the binary packages produced by the Debian
                # source
        ...
        if len(setup_env_vars):
            ...
            self.exports += '\n'.join(
                ['export %s' % v for v in setup_env_vars])
            self.exports += '\n'
        ...
        if with_python3:
            ...
            if self.distname3 and self.distname3 != self.distname:
                if with_python2:
                    raise ValueError(
                        "Suites are shared between versions. To use a "
                        "different value for Suite3 run --with-python3 true "
                        "--with-python2 false only.")
                self.changelog_distname = \
                    CHANGELOG_PY3_DISTNAME % self.__dict__
        else:
...
def build_dsc(debinfo,
    ...
    if debian_dir_only:
        ...
        fullpath_repackaged_dirname = os.path.join(
            dist_dir, repackaged_dirname)
